...
def see_progress(update, context):
    ...
    if chat_id in np.array(pd.read_sql("SELECT chat_id FROM users", sqlite3.connect("/home/iqrorjon/PycharmProjects/astrum-bot/database/user_datas.db")).values).flatten():
        data = pd.read_sql(f"SELECT qwasar_username FROM users WHERE chat_id={chat_id}", sqlite3.connect("/home/iqrorjon/PycharmProjects/astrum-bot/database/user_datas.db"))['qwasar_username'][0]
        update.message.reply_text(data)

        df = pd.read_csv("/home/iqrorjon/PycharmProjects/astrum-bot/database/data.csv")

...
def username(update, context):
    ...
    if message in np.array(pd.read_sql("""SELECT qwasar_username FROM users""", sqlite3.connect("/home/iqrorjon/PycharmProjects/astrum-bot/database/user_datas.db"))).flatten():
...
def submit(update, context):
    message = update.message.text
    if message == pd.read_sql(f"SELECT parol FROM users WHERE qwasar_username='{context.user_data['qwasar_username']}'", sqlite3.connect("/home/iqrorjon/PycharmProjects/astrum-bot/database/user_datas.db"))['parol'][0]:
